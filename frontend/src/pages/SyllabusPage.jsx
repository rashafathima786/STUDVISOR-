      ...
      <div className="max-w-5xl mx-auto space-y-10 pb-20">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 relative z-10">
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                 ...
                 <h2 className="text-3xl font-black text-on-surface tracking-tight">Curriculum Mastery</h2>
               </div>
               <p className="text-on-surface-variant/50 max-w-md text-sm leading-relaxed">
                 You have successfully completed <span className="text-on-surface font-bold">{overallProgress}%</span> of your total course objectives for the current semester.
               </p>
            </div>
           ...
           <div className="mt-10 h-3 bg-surface-container rounded-full overflow-hidden border border-border-color">
            <motion.div 
              initial={{ width: 0 }}
        ...
        <div className="space-y-8">
           <div className="flex items-center gap-3 px-2">
             ...
             <h2 className="text-2xl font-bold text-on-surface tracking-tight">Active Subjects</h2>
           </div>
                  ...
                  return (
                    <motion.div
                      key={subject.subject_id}
                       ...
                       className="glass-panel rounded-[32px] overflow-hidden border border-border-color group hover:border-primary/30 transition-all duration-500"
                     >
                      <div className="p-8 sm:p-10 space-y-8">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                          <div className="flex items-center gap-4">
                           ...
                           <div className="text-right">
                              <div className={`text-3xl font-black tracking-tighter ${progress === 100 ? 'text-tertiary' : 'text-on-surface'}`}>
                                {progress}%
                              </div>
                        ...
                        <div className="flex gap-1.5 h-2">
                           {subject.topics.map((topic, i) => (
                             <div 
                               key={topic.id} 
                               className={`flex-grow rounded-full transition-all duration-500 ${topic.completed ? (progress === 100 ? 'bg-tertiary shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'bg-primary') : 'bg-surface-container-high'}`}
                             />
                           ))}
                        </div>
                            ...
                            <motion.div 
                              ...
                              onClick={() => handleToggle(topic.id)}
                               className={`
                                 ...
                                 ${topic.completed 
                                   ? 'bg-surface-container border-border-color opacity-60' 
                                   : 'bg-surface-container/50 border-border-color hover:border-primary/40 hover:bg-surface-container'}
                               `}
                            >
                              <div className={`transition-colors ${topic.completed ? 'text-tertiary' : 'text-on-surface-variant/20'}`}>
                               ...
                               <span className={`text-sm font-medium transition-all ${topic.completed ? 'text-on-surface-variant/40 line-through' : 'text-on-surface'}`}>
                                 {topic.name}
                               </span>
                              {!topic.completed && (
