...
export default function LostFoundPage() {
  ...
  const handlePostItem = async (e) => {
    ...
    try {
      await createLostFoundItem({ item_name: title, description, type: itemType, location });
      setShowModal(false);
    ...
    <ErpLayout title="Lost & Found" subtitle="Community-driven item recovery portal">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
          <div className="flex items-center gap-2 bg-surface-container p-1 rounded-2xl border border-border-color">
        ...
        <div className="min-h-[400px]">
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3, 4, 5, 6].map(i => (
                <div key={i} className="h-64 rounded-3xl bg-surface-container animate-pulse" />
              ))}
            </div>
          ) : items.length === 0 ? (
            <motion.div 
            ...
            >
              <div className="w-20 h-20 rounded-full bg-surface-container flex items-center justify-center mb-6">
                <Search size={32} className="text-on-surface-variant/30" />
              </div>
              <h3 className="text-xl font-bold text-on-surface mb-2">No Entries Found</h3>
              <p className="text-on-surface-variant/60 text-sm max-w-xs">There are no {filter !== 'all' ? filter : ''} items reported in this sector.</p>
            </motion.div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <AnimatePresence mode="popLayout">
                {items.map((item) => (
                  ...
                  >
                    ...
                    <h3 className="text-lg font-bold text-on-surface mb-3 tracking-tight group-hover:text-primary transition-colors">
                      {item.item_name}
                    </h3>
                    
                    <p className="text-on-surface-variant/70 text-sm leading-relaxed mb-6 flex-grow line-clamp-3">
                      {item.description}
                    </p>
                    
                    <div className="space-y-3 pt-6 border-t border-border-color">
                      <div className="flex items-center gap-3 text-on-surface-variant/50 text-[11px] font-bold uppercase tracking-wider">
                    ...
                    <button 
                      ...
                      className={`mt-6 w-full py-4 rounded-2xl font-bold text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 group/btn ${
                        contactedId === item.id 
                          ...
                          : 'bg-surface-container-high hover:bg-primary text-on-surface hover:text-white border border-border-color'
                      }`}
                    >
                      {contactedId === item.id ? (
                        <><span>Request Sent</span><CheckCircle size={14} /></>
                      ) : (
                        <><span>Contact Authority</span><ArrowRight size={14} className="group-hover/btn:translate-x-1 transition-transform" /></>
                      )}
                    </button>
                  </motion.div>
                ))}
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
              ...
              >
                <div className="flex justify-between items-center mb-10">
                  <div>
                    <h2 className="text-2xl font-bold text-on-surface mb-2">Report Entry</h2>
                    <p className="text-on-surface-variant/60 text-sm">Synchronizing item details with the central node.</p>
                  </div>
                ...
                <form onSubmit={handlePostItem} className="space-y-6">
                  <div className="grid grid-cols-2 gap-2 bg-surface-container p-1 rounded-2xl border border-border-color">
                    {['lost', 'found'].map((t) => (
                      <button
                  ...
                  {[
                    { label: 'Object Identifier', value: title, set: setTitle, placeholder: 'e.g. BLUE SONIC EARBUDS', type: 'text' },
                    { label: 'Last Known Coordinates', value: location, set: setLocation, placeholder: 'e.g. SECTOR-4 CAFE', type: 'text' },
                  ].map(({ label, value, set, placeholder, type }) => (
                    <div key={label} className="space-y-2">
                      <label className="text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-widest ml-1">{label}</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-widest ml-1">Object Description</label>
                    <textarea required value={description} onChange={e => setDescription(e.target.value)}
                      placeholder="Provide high-fidelity details..." rows="3"
                      className="w-full bg-surface-container border border-border-color rounded-2xl p-4 text-on-surface outline-none focus:border-primary/50 transition-all placeholder:text-on-surface-variant/30 resize-none" />
                  </div>

                  <div className="flex gap-4 pt-6">
                    <button type="button" onClick={() => setShowModal(false)}
                      className="flex-1 py-4 rounded-2xl bg-surface-container text-on-surface-variant/60 font-bold text-xs uppercase tracking-widest hover:bg-surface-container-high transition-all border border-border-color">
                      Abort
                    </button>
                    <button type="submit"
